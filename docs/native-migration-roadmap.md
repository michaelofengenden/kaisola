...
is the authoritative parity checklist for the remaining native follow-through:
every user-facing Electron capability,
...
- **MCP**: per-workspace server registry producing the `session/new` mcpServers array with correct stdio/http/sse shapes, capability-filtered; Settings can safely discover known sibling-client configs for explicit disabled import and probe Streamable HTTP through the negotiated initialize/initialized/tools lifecycle (`McpConfigStore.swift`, `scripts/native-mcp-registry.cjs`).
...
| Session tabs / dock-grid (draggable columns, split, close, pop) | DONE | P0 | in-window splits (up to 4 panes, each its own live subscription; owned panes fully interactive), tab bar with promote/close, Open in Split context menu, edge-aware pointer drag-reorder, pop-out to window |
...
| Fonts (family/size ⌘±/weight/line-height) | DONE | P1 | size ⌘+/⌘−/⌘0 + family/weight/line-spacing pickers persist and update live |
...
| Rename / auto-name / prompt title | DONE | P1 | manual rename + agent·folder creation name + protected live OSC/process auto-title until manually renamed |
| Agent detection / meta (process, cwd, branch, ports, exit) | DONE | P0 | agent id + live activity/responded/exit + cwd-backed git branch + foreground process + listening ports on rows (bounded TTL scans) |
...
| Blocks / OSC 133/633 prompt marks | PARTIAL | P2 | Public SwiftTerm handlers rebuild a bounded replay-safe command index and power safe navigation/context-menu actions; default-off, dotfile-free zsh, Bash, and Fish wrappers emit prompt/command marks, while modern Fish keeps its native markers and passive phase-colored command separators have dedicated light/dark fixtures. Provider-composer boundaries and live theme/session QA remain. |
| CLI draft survival (retype into resumed agent) | DONE | P2 | owner-only bounded drafts survive GUI replacement; reopening a prepared Claude/Codex/OpenCode/Gemini terminal resumes the provider in the same project and immutable account context, then retypes only after startup quiet; terminal protocol replies are excluded and real interaction cancels restore. Continuation is deliberately best-effort most-recent provider state, not an invented exact CLI session ID |
...
| Terminal (font/size/weight/line-height/tone/cursor) | DONE | P1 | font-size + family/weight/line-spacing and macOS Terminal/Kaisola palette pickers persist and update live |
| Agents (add custom terminal/ACP, enable presets, models) | PARTIAL | P1 | adapter roster + app-wide/project account isolation + immutable per-session Claude/Codex binding across terminal, chat, reopen, workspace restore, and Mesh continuation + custom agent registration + device-code sign-in card |
| Models & keys (API keys keychain, provider, base URLs) | DONE | P1 | Keychain-backed Anthropic/OpenAI keys plus a provider picker, validated custom HTTPS/local URLs, per-provider models, direct-API environment overlays, documented Codex one-run CLI overrides, and equivalent `codex-acp` `CODEX_CONFIG`; user `config.toml` is never rewritten |
| Usage (Codex/Claude/OpenCode gauges, limits) | DONE | P1 | provider plan limits, per-chat context, persisted session totals, turns, currency-separated cost totals, and inline chat/session/footer cost chips |
| Interface (cost chips, inbox, diffs, drafts, nav, perf) | DONE | P1 | cost chips; durable inbox, dock badge, and native notifications; word-level unified/split diffs; drafts; toasts; both nav layouts; deterministic visual and resource gates |
| Companion (pairing/devices) | PARTIAL | P2 | native opt-in, QR/code offer, Noise XX + SAS confirmation, capability grant, paired-device roster/revoke, Nearby and Link diagnostics ship in the working tree; real iPhone/network/release gates remain |
...
| Feature | Status | Pri | Notes |
|---|---|---|---|
| Claude accounts (isolated CLAUDE_CONFIG_DIR, per-project/session) | DONE | P1 | app-wide + per-project scoping; new-session picker snapshots the canonical config directory and locks it through terminal/chat continuation, close/reopen, workspace restore, and Mesh |
| Codex accounts (isolated CODEX_HOME, per-project/session) | DONE | P1 | app-wide + per-project scoping; new-session picker snapshots the canonical config directory and locks it through terminal/chat continuation, close/reopen, workspace restore, and Mesh |
...
| MCP servers (per-workspace, add/probe/import, carried into agents) | DONE | P2 | per-workspace registry in Settings, exact Electron wire shapes carried into chats and Mesh, bounded known-client discovery with explicit disabled import, non-spawning stdio/SSE status, and negotiated Streamable HTTP health/tool probes |
| Extensions | NEW | P2 | |
| App account (Google/Firebase, for companion rendezvous) | DONE | P2 | shared iPhone Firebase REST/Keychain layer; Google browser sign-in, restore/sign-out, profile name/email, and avatar in Settings/footer; remembered-session caches and suspended UI updates are fenced to the initiating Firebase UID across Mac, iPhone, and web; a Developer-ID-only broker-free catalog smoke exercises synthetic publish/exact read/remove/absence with no token output—the existing Electron account proved that signed Swift client against production through an anonymous in-memory pipe, while the native Keychain restore route honestly remains `AUTH_REQUIRED` pending one deliberate production sign-in |

## 7. Companion / mobile

Protocol + crypto are shared in `KaisolaCore`; the native desktop host now owns
pair/resume, bounded projection/replay, terminal observation/control leases,
Nearby transport, and the blind Link route. These rows remain PARTIAL until the
real-device, account-isolation, network-switch, and distribution gates in the
working backlog pass.

| Feature | Status | Pri |
|---|---|---|
| Pairing (QR/phrase, Noise-XX, account rendezvous) | PARTIAL | P2 |
| Projection publishing (redacted state to phone) | PARTIAL | P2 |
| Device management (list/rename/revoke, capabilities) | PARTIAL | P2 |
...
| Feature | Status | Pri | Notes |
|---|---|---|---|
| Auto-updates (Sparkle) | DONE | P1 | signed client/update path and real update verified; automatic tag-generated appcast publication activation is tracked separately in the backlog |
...
| Wallpaper tint sampling | DONE | P2 | Tinted mode downsamples the active desktop image off the UI thread and cools its average toward indigo |
| Window mode / traffic lights / relaunch | DONE | P1 | native full-size-content windows, explicit sidebar traffic-light clearance, saved frames, multi-window behavior, and user-confirmed update relaunch that names interruptible chats while preserving broker terminals |
...
| Cost / usage chips | DONE | P2 | Settings exposes provider limits, context/session totals, turns, and currency-separated cost totals; chat controls, session navigation, and the footer expose cumulative cost without mixing currencies |
...
**Status 2026-07-25: native Kaisola is the shipping desktop product.** The
Electron shell is archived and no longer built by the release workflow. The
native parity campaign closed the shell and agent-workspace cutover, including
stable pointer resizing, rendered-Markdown editing, wallpaper-derived tinting,
and the shared Google/Firebase account. The working tree also contains the
native Companion host through Nearby and Link, but real-device/account/network
hardening remains. Other remaining product work includes OSC 133 prompt blocks,
accessibility/interaction evidence, and Phase F research legacy
scope.
...
- **Phase E — integrations & long tail (P2):** companion host; MCP + extensions; PDF/LaTeX preview; browser cards; ledger; workflows; OmniBar; keymap; project detach/adopt.
