...
final class CompanionStore: ObservableObject {
    ...
    func apply(_ envelope: CompanionEnvelope) throws -> Bool {
        ...
        case .snapshot:
            if envelope.body.type == "terminal.snapshot" {
                // Protocol v1 permits terminal.snapshot as either a snapshot
                // or event envelope. The original Electron host emits an
                // event; accept the other legal spelling so a mixed-version
                // desktop cannot make the phone stale during subscription.
