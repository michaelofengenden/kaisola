...
# development app. Raw Xcode products live in a .noindex DerivedData folder so
...
build_current_source() {
  ...
  # Stop only this exact development bundle; production Kaisola and every
  # broker-owned PTY remain untouched.
  stop_development_app
  /bin/rm -rf "$APP"
  /bin/mv "$staging" "$APP"
...
# visually identical intermediate builds even though only one is installed.
...
# build directories remain untouched.
...
if [[ "$BUILD_CURRENT" -eq 1 ]]; then
  ...
  /bin/echo "Kaisola development app not found at: $APP" >&2
