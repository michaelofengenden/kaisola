...
///
/// Ordinary contract tests use an in-memory implementation of the exact
/// KeychainAccess seam. Only KeychainBoundaryTests below may touch the
/// data-protection Keychain, and they require the dedicated signed lane.
final class ApiKeyStoreTests: XCTestCase {
    private var service = ""
    private var store = ApiKeyStore()
    private var backend: ApiKeyStoreTestBackend!

    override func setUp() {
        super.setUp()
        service = "com.kaisola.mac.preview.api-keys.test-\(UUID().uuidString)"
        backend = ApiKeyStoreTestBackend()
        store = ApiKeyStore(service: service, keychain: backend.access)
    }
