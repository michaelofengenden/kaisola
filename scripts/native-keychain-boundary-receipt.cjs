...
function parseSignature(text) {
  ...
  let kind = null
  if (/^Signature=adhoc$/mu.test(value)) kind = 'adhoc'
  else if (/^Authority=Apple Development:/mu.test(value)) kind = 'apple-development'
  ...
  if (kind === 'adhoc') {
    if (teamIdentifier !== 'not set') fail('ad-hoc host must not claim a signing team')
  } else if (!/^[A-Z0-9]{10}$/u.test(teamIdentifier || '')) {
...
function validateEntitlements(entitlements, signature) {
  ...
  const expectedApplicationIdentifier = signature.kind === 'adhoc'
    ? EXPECTED_IDENTIFIER
    : `${signature.teamIdentifier}.${EXPECTED_IDENTIFIER}`
...
function validateBoundaryEvidence(evidence) {
  ...
  validateEntitlements(evidence.host?.entitlements, parsedSignature)
