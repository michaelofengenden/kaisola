...
class TerminalSpool {
  ...
  _noteQuotaPressure(currentSize, budget) {
    const retained = currentSize + segmentSize(this.prevFile)
  ...
  _rotateSegments(quotaDriven) {
    ...
    const discardedBytes = discarded ? segmentSize(this.prevFile) : 0
    ...
    try {
      for (const name of fs.readdirSync(this.dir)) {
        ...
        try {
          ...
          if (stat.isFile()) segments.push({ file, name, size: stat.size, mtimeMs: stat.mtimeMs })
  ...
  _retainedDiskBytes() {
    return segmentSize(this.prevFile) + segmentSize(this.file)
  }
