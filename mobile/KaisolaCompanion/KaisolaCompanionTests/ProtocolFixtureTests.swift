...
final class ProtocolFixtureTests: XCTestCase {
    ...
    @MainActor
    func testTerminalSnapshotIsAcceptedInEitherProtocolLegalEnvelopeKind() throws {
        let store = try liveStoreFromSnapshot()
        let envelope = try CompanionEnvelope(
            kind: .snapshot,
            desktopId: "desktop-fixture",
            ...
            epoch: "desktop-epoch-7",
            seq: 13,
            id: "terminal-snapshot-13",
            sentAt: 1_784_250_001_213,
            body: CompanionBody(fields: [
                "type": .string("terminal.snapshot"),
                "projectId": .string("project-kaisola"),
                "terminalId": .string("session-done"),
                "streamEpoch": .string("terminal-epoch-snapshot"),
                "endOffset": .integer(5),
                "output": .string("ready"),
            ])
        )
        ...
        let terminal = try XCTUnwrap(store.session(for: "session-done"))
        XCTAssertEqual(terminal.terminalOutput, "ready")
        XCTAssertEqual(terminal.terminalStreamEpoch, "terminal-epoch-snapshot")
        XCTAssertEqual(terminal.terminalEndOffset, 5)
        XCTAssertEqual(store.lastAckCursor, CompanionAckCursor(epoch: "desktop-epoch-7", seq: 13))
    }
    ...
    /// checked-in canonical runtime/node-broker/companion/protocolTables.json, so a kind /
    ...
    private func canonicalTables() throws -> ProtocolTables {
        ...
        let url = repoRoot
            .appendingPathComponent("runtime/node-broker/companion/protocolTables.json")
