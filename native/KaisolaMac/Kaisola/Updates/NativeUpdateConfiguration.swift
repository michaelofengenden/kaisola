...
enum NativeUpdateConfigurationError: Error, Equatable, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notConfigured:
            "Kaisola updates are not configured in this build."
        case .unsafeFeedURL:
            "The Kaisola update feed must be an HTTPS URL without credentials or a fragment."
        case .invalidPublicKey:
            "The Kaisola update signing key is invalid."
        }
    }
}
