\- when switching between windows, tabs takes a second or two to load (it should be flawless and fast)

\- fair a robust memory check for the app, make sure it's incredibly efficient without losing any functionality!

\- the settings bar:

![Screenshot 2026-07-15 at 5.23.05 PM](backlog-media/Screenshot%202026-07-15%20at%205.23.05%E2%80%AFPM.png#kaisola-width=58)

can be fixed further as well, the settings can just included inside the profile of the user, and the notifications and search button need to be on the same level. 

\-  Also images inside md files should be more easily movable and deletable and writing aroundable. For example the image below is stuck and I can't write text underneath it. 

also the mesh is not bad, we can reduce amount of text though, remember we want minamilism, also the effort menu should be for codex as well, also I want there to be an added mode, say like Idea mode where I ask the agents what they think about some kind of idea and then one agent answers then another, then they both answer on each other's responses and on my input. let both agents respond first to initial prompt but then prior messages should be incorporated into their own prompting/steering if an agent responds faster than another and so forth.... very groupchaty

![image](backlog-media/image.png#kaisola-width=48)

![Screenshot 2026-07-15 at 5.23.05 PM](backlog-media/Screenshot%202026-07-15%20at%205.23.05%E2%80%AFPM.png#kaisola-width=20)
