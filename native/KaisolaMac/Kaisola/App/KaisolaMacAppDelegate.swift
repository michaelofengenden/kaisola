...
final class KaisolaMacAppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, NSMenuDelegate, NSMenuItemValidation {
    ...
    private func makeWindow(
        ...
        if visualSettings || visualOnboarding {
            ...
            // The standard window buttons are AppKit's, untouched. A
            // repositioning controller lived here through 0.1.147–0.1.149 to
            // create a Safari-style inset; measurement ended it. The system
            // already insets the buttons on toolbar-carrying windows, stock
            // layouts differ across real windows (16pt buttons at 18/41/64 on
            // one machine's workspace window, 12pt at another pitch on
            // another's), and all three shipped attempts to move them produced
            // uneven spacing that AppKit then could not repair because our
            // positions masked its own. The buttons cannot be wrong if nothing
            // moves them.
        }
