...
test('cross-renderer project moves fail closed before any ownership handoff', () => {
  ...
  const disabledGuard = body.indexOf('if (!CROSS_RENDERER_PROJECT_TRANSFERS_ENABLED)', sameReturn)
  const acpHandoff = body.indexOf('transferAcpProject(', disabledGuard)
  const adoptionWindow = body.indexOf('createWindow({ slot: freeSlot(), adopt: true', disabledGuard)

  assert.match(source, /const CROSS_RENDERER_PROJECT_TRANSFERS_ENABLED = false/)
  assert.ok(physicalMove >= 0 && physicalMove < sameReturn)
  assert.ok(sameReturn < disabledGuard)
  assert.ok(disabledGuard < acpHandoff)
  assert.ok(disabledGuard < adoptionWindow)
})
