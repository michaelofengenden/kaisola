...
final class AuthModel: ObservableObject {
    ...
    /// Optional safe copy explaining why an otherwise normal signed-out state
    /// needs one deliberate sign-in. The Mac uses this for its one-time move
    /// from ad-hoc Keychain identity to a stable Developer ID identity; the
    /// iPhone leaves it nil.
    @Published private(set) var signedOutNotice: String?

    private let backend: AuthBackend

    init(backend: AuthBackend, signedOutNotice: String? = nil) {
        self.backend = backend
        self.signedOutNotice = signedOutNotice
    }
    ...
    func restore() async {
        do {
            ...
            // A locked or migrated Keychain item must not look like a clean
            // signed-out account: that hides why remembered sessions and Link
            // disappeared. Keep the recovery action and the safe error copy
            // visible without exposing any credential material.
...
extension AuthModel {
    ...
    static func previewSignedOut(notice: String? = nil) -> AuthModel {
        let model = AuthModel(
            backend: PreviewAuthBackend(startSignedIn: false),
