...
final class ExtensionsSettingsHubTests: XCTestCase {
    ...
    func testUnreadableGrammarRegistryIsASecretFreeActionableCatalogRow() {
        let preservedURL = URL(fileURLWithPath: "/private/tmp/custom-grammars.json.preserved-deadbeef.json")
        let state = CustomGrammarStore.LoadState.corrupt(.preserved(preservedURL))
        let item = ExtensionSettingsItem.languageGrammarRegistryIssue(state)

        XCTAssertEqual(item.id, CustomGrammarStore.registryIssueID)
        XCTAssertEqual(item.category, .languageGrammars)
    ...
    func testGrammarRegistryIOFailureCannotOfferDestructiveRecoveryOrLeakItsPath() {
        let state = CustomGrammarStore.LoadState.ioFailure("permission denied at /secret/location")
        let item = ExtensionSettingsItem.languageGrammarRegistryIssue(state)
