...
export function SignInCard() {
  ...
  return (
    <dialog
      ...
      style={SIGN_IN_DIALOG_STYLE}
      aria-labelledby="signin-title"
      onCancel={(event) => { event.preventDefault(); close() }}
    >
      <div ref={panelRef} className="signin-card" tabIndex={-1}>
        <header className="focus-head">
          <Icon name="KeyRound" size={14} className="muted" />
          <span className="grow" id="signin-title">Sign in to {signIn.name}</span>
          <button type="button" className="btn-icon btn-sm" onClick={close} aria-label="Close sign in"><Icon name="X" size={14} /></button>
        </header>
        ...
        <div className="signin-body">
          {phase === 'done' ? (
            ...
            <>
              ...
              {ev?.url && <button type="button" className="signin-link mono faint" onClick={() => bridge.openExternal(ev.url!)}>{ev.url}</button>}

              <button type="button" className="btn btn-ghost btn-sm signin-cancel" onClick={close}>Cancel</button>
            </>
          )}
        </div>
      </div>
