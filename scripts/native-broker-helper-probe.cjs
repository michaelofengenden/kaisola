...
const CLIENT_DEADLINE_MS = 8_000
...
class Client {
  ...
  async connect() {
    return await new Promise((resolve, reject) => {
      ...
      const deadline = setTimeout(() => {
        socket.destroy()
        finish(new Error(`broker hello timed out after ${CLIENT_DEADLINE_MS}ms (${this.access})`))
      }, CLIENT_DEADLINE_MS)
      ...
      socket.on('data', (chunk) => {
        ...
        while ((newline = this.buffer.indexOf('\n')) >= 0) {
          ...
          if (!authenticated && frame.type === 'hello') {
            if (!frame.ok) finish(new Error(frame.message || 'broker hello rejected'))
            else { authenticated = true; finish(null, frame) }
            ...
            else pending.reject(new Error(`${pending.method} failed: ${frame.message || 'broker request failed'}`))
      ...
      socket.once('close', () => {
        ...
        finish(new Error(`broker closed before hello (${this.access})`))
        this.rejectPending(new Error('broker closed with requests pending'))
      })
    })
  }
  ...
  request(method, params = {}) {
    ...
    return new Promise((resolve, reject) => {
      if (!this.socket || this.socket.destroyed) {
        reject(new Error(`${method} cannot run on a closed broker client`))
        return
      }
      const deadline = setTimeout(() => {
        if (!this.pending.delete(id)) return
        reject(new Error(`${method} timed out after ${CLIENT_DEADLINE_MS}ms`))
      }, CLIENT_DEADLINE_MS)
      this.pending.set(id, { resolve, reject, deadline, method })
  ...
  close() {
    activeClients.delete(this)
    this.rejectPending(new Error('broker client closed'))
    this.socket?.destroy()
    this.socket = null
  }
}
...
;(async () => {
  ...
  const observerN = new Client('observer', 'kaisola-native-N')
  ...
  // A timed-out request may leave an authenticated socket open. The probe is a
  // bounded release gate, so discard every partial client before creating the
  // separate best-effort shutdown connection.
  closeActiveClients()
  try {
