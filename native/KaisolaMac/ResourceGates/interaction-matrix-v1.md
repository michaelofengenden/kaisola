...
> Historical Phase-1 snapshot. Superseded by
> [interaction-matrix-v2.md](interaction-matrix-v2.md); do not use the status
> labels below as current release evidence.
...
| Selection, copy, and paste | The view claims first responder on window entry; `NativeTerminalInteractionTests` pins main-menu and contextual Copy/Paste/Select All wiring and verifies Codex-style Unicode multiline bracketed paste byte-for-byte; SwiftTerm writes selection to the general pasteboard directly | AUTOMATED PASS (wiring/protocol) | Mouse drag selection, Command-C/Command-V, contextual copy/paste, and large Unicode paste in real Codex on the packaged artifact |
