...
enum QuietSelectionPill {
    ...
    /// 2026-08-28 feedback — "I'd like there'd not to be a double card
    /// situation for the lhs rail" — graduated: ONLY the selected row wears a
    /// fill (Finder's and Safari's grammar), so a split's companion pane and
    /// every other resting row are quiet text and icon inside the one rail
    /// surface, answering the pointer with the faint wash. `isOnScreen`
    /// stays in the signature because the rule it feeds is exactly what the
    /// contract test pins: an on-screen companion row rests quiet.
    static func fill(
        isSelected: Bool,
        isOnScreen: Bool,
        hovering: Bool
        ...
        return hovering ? .hoverWash : .none
    }
...
private struct QuietSelectionPillView: View {
    ...
    var body: some View {
        // The tab silhouette at row scale, so the rail's selection and the
        // session tabs speak one shape language.
...
/// The rows' pointer answer: the same shape as the selection pill, in a
/// faint neutral wash — the session tabs' hover fill at row scale.
...
private struct QuietRowBody: View {
    ...
    var body: some View {
        ...
        // hidden from it. Graduated grammar (2026-08-28): only the selected
        // row is filled inside the rail, everything else — a split's
        // companion pane included — rests quiet, and hover answers with the
        // tabs' faint wash. `showsReveal` is the row's hover flag; the wash
        // eases in and out on it instead of flipping.
        .background {
            let fill = QuietSelectionPill.fill(
