...
final class BrokerDiscoveryTests: XCTestCase {
    ...
    func testPreviewDefaultsToItsNativeOnlyProfileInEveryBuild() {
    ...
    func testResourceWorkloadAcceptsOnlyNamedPrivateTemporaryRoots() {
        let environment = [
            "KAISOLA_NATIVE_RESOURCE_WORKLOAD": NativeResourceWorkloadConfiguration.streamingID,
            "KAISOLA_NATIVE_RESOURCE_ROOT": root.appendingPathComponent("paired").path,
        ]
        let configuration = NativeResourceWorkloadConfiguration.resolve(
            environment: environment,
            temporaryDirectory: root.deletingLastPathComponent()
        )
        XCTAssertEqual(configuration?.workloadID, NativeResourceWorkloadConfiguration.streamingID)
        XCTAssertEqual(configuration?.brokerUserDataRoot.lastPathComponent, "broker-profile")
        XCTAssertEqual(configuration?.appStateRoot.lastPathComponent, "app-state")
        XCTAssertEqual(configuration?.workspaceRoot.lastPathComponent, "workspace")
        XCTAssertEqual(configuration?.expectedWindowCount, 1)

        let restored = NativeResourceWorkloadConfiguration.resolve(
            environment: [
                "KAISOLA_NATIVE_RESOURCE_WORKLOAD": NativeResourceWorkloadConfiguration.restoredWindowsID,
                "KAISOLA_NATIVE_RESOURCE_ROOT": root.appendingPathComponent("restored").path,
            ],
            temporaryDirectory: root.deletingLastPathComponent()
        )
        XCTAssertEqual(restored?.expectedWindowCount, 3)
        XCTAssertEqual(
            restored?.restoredWorkspaceRoots.map(\.lastPathComponent),
            ["workspace-1", "workspace-2", "workspace-3"]
        )
    }

    func testResourceWorkloadFailsClosedOutsideTemporaryRootOrForUnknownWorkload() {
        XCTAssertNil(NativeResourceWorkloadConfiguration.resolve(
            environment: [
                "KAISOLA_NATIVE_RESOURCE_WORKLOAD": NativeResourceWorkloadConfiguration.idleID,
                "KAISOLA_NATIVE_RESOURCE_ROOT": "/Users/example/production",
            ],
            temporaryDirectory: root
        ))
        XCTAssertNil(NativeResourceWorkloadConfiguration.resolve(
            environment: [
                "KAISOLA_NATIVE_RESOURCE_WORKLOAD": "not-a-workload",
                "KAISOLA_NATIVE_RESOURCE_ROOT": root.appendingPathComponent("paired").path,
            ],
            temporaryDirectory: root
        ))
    ...
    func testNativeFrameCadenceSummaryPassesAContinuousDisplaySequence() throws {
        let frame = 1.0 / 60.0
        let timestamps = (0...1_800).map { Double($0) * frame }
        let report = try XCTUnwrap(NativeFrameCadenceReport.summarize(
            workload: NativeResourceWorkloadConfiguration.streamingID,
            callbackTimestamps: timestamps,
            nominalFrameDurations: Array(repeating: frame, count: timestamps.count)
        ))

        XCTAssertTrue(report.pass)
        XCTAssertEqual(report.missedFrameCount, 0)
        XCTAssertEqual(report.deadlineLossRateMsPerSecond, 0)
        XCTAssertGreaterThanOrEqual(report.callbackCoverage, 0.99)
    }

    func testNativeFrameCadenceSummaryFailsSkippedAndLongFrames() throws {
        let frame = 1.0 / 60.0
        var timestamps = (0...1_800).map { Double($0) * frame }
        for index in stride(from: 1_700, through: 100, by: -10) {
            timestamps.remove(at: index)
        }
        timestamps.append(timestamps.removeLast() + 0.12)
        let report = try XCTUnwrap(NativeFrameCadenceReport.summarize(
            workload: NativeResourceWorkloadConfiguration.streamingID,
            callbackTimestamps: timestamps,
            nominalFrameDurations: Array(repeating: frame, count: timestamps.count)
        ...
        XCTAssertGreaterThan(report.missedFrameCount, 0)
        XCTAssertFalse(report.checks.maximumInterval)
