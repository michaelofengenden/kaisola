...
function readPs() {
  ...
  try {
    const raw = execFileSync('/bin/ps', ['eww', '-axo', 'pid=,pgid=,command='], { encoding: 'utf8', maxBuffer: 32 * 1024 * 1024 })
    return raw.split('\n').map((line) => {
...
class AcpProcessLedger {
  constructor(dir, deps = {}) {
    ...
    this.records = Array.isArray(prior.records) ? prior.records.filter((r) => r && r.token && r.instanceId) : []
    this.reclaimed = []
    this._save()
  }

  _save() {
    try { atomicWrite(this.file, { version: 1, ownerId: this.ownerId, records: this.records }) } catch { /* diagnostics must never block agents */ }
  }
  ...
  recordExit(token) {
    ...
    if (this.scan().some((r) => r.command.includes(owner) && r.command.includes(instance) && r.command.includes(conn))) return
      ...
      try {
        ...
        if (this.hardKillDelay >= 0) {
          const hard = setTimeout(() => {
            const remaining = this.scan().filter((r) => r.command.includes(owner) && r.command.includes(instance) && r.command.includes(conn))
